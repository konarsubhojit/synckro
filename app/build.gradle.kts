...
/**
 * Generates a plain-text list of the OSS dependencies pulled into the runtime
 * classpath. The output is bundled as `assets/oss_licenses.txt` and surfaced
 * by `SettingsViewModel.loadOssLicensesText()` in the About section.
 *
 * Intentionally lightweight: this task only enumerates the resolved Maven
 * coordinates (group:artifact:version) so the build does not depend on the
 * Play Services OSS license library or any network resolution of POM
 * `<licenses>` blocks at build time. License terms for each dependency remain
 * the responsibility of the upstream project; this file gives users (and
 * auditors) a single place to discover what to look up.
 */
abstract class GenerateOssLicensesTask : DefaultTask() {
    ...
    @TaskAction
    fun execute() {
        val sorted = coordinates.get().distinct().sorted()
        val header =
            buildString {
                appendLine("Synckro — Open-source dependencies")
                appendLine("Generated by the :app:generate<Variant>OssLicenses Gradle task.")
                appendLine("Refer to each project's homepage for the full license text.")
                appendLine()
                appendLine("Total: ${sorted.size}")
                appendLine()
            }
        val outFile = outputDir.get().asFile.resolve("oss_licenses.txt")
        outFile.parentFile.mkdirs()
        outFile.writeText(header + sorted.joinToString(separator = "\n"))
    }
}
...
androidComponents {
    onVariants { variant ->
        ...
        // OSS licenses: enumerate runtime-classpath coordinates at configuration
        // time (cheap — Gradle already resolved them) and snapshot them into the
        // task input so it's deterministic and cache-friendly. The Configuration
        // we read from is the per-variant `<variantName>RuntimeClasspath`.
        val runtimeConfigName = "${variant.name}RuntimeClasspath"
        val runtimeCoordsProvider =
            providers.provider {
                configurations.findByName(runtimeConfigName)
                    ?.incoming
                    ?.resolutionResult
                    ?.allComponents
                    ?.mapNotNull { comp ->
        ...
        val generateOssLicensesForVariant =
            tasks.register("generate${variantCap}OssLicenses", GenerateOssLicensesTask::class) {
                coordinates.set(runtimeCoordsProvider)
                outputDir.set(layout.buildDirectory.dir("generated/assets/oss/${variant.name}"))
            }
