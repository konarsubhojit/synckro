...
class GoogleDriveAuthManagerTest {
    ...
    @Test
    fun `acquireAccessToken returns NotConfigured when not configured`() = runTest {
        val account = Account(
            id = "test@example.com",
    ...
    @Test
    fun `currentAccounts returns empty list when no account is stored`() = runTest {
        assertTrue(authManager.currentAccounts().isEmpty())
    }

    @Test
    fun `currentAccounts returns stored account after prefs are populated`() = runTest {
        testPrefs.edit()
    ...
    @Test
    fun `signOut clears stored account and returns Success`() = runTest {
        testPrefs.edit()
            .putString("account_id", "user@gmail.com")
            .putString("display_name", "Test User")
            .putString("email", "user@gmail.com")
    ...
    @Test
    fun `currentAccounts falls back to id when display_name is missing`() = runTest {
        testPrefs.edit()
            .putString("account_id", "user@gmail.com")
