...
class GoogleDriveRemoteEnumeratorTest {
    ...
    @Test
    fun `enumerate null fetches startPageToken and returns empty baseline`() = runTest {
        server.enqueue(
            MockResponse()
                .setResponseCode(200)
    ...
    @Test
    fun `enumerate with token maps modified, removed, and trashed entries`() = runTest {
        server.enqueue(
            MockResponse()
                .setResponseCode(200)
                .setBody(
    ...
    @Test
    fun `enumerate follows nextPageToken before returning newStartPageToken`() = runTest {
