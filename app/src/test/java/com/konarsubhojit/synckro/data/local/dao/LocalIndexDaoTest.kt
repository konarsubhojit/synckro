...
class LocalIndexDaoTest {
    ...
    private suspend fun insertPair(id: Long = 0L): Long = pairDao.insert(
        SyncPairEntity(
            id = id,
    ...
    @Test
    fun `upsert inserts entry and getForPair returns it`() = runTest {
    ...
    @Test
    fun `upsert updates existing entry on same pairId and relativePath`() = runTest {
        val pairId = insertPair()
    ...
    @Test
    fun `upsertAll inserts multiple entries`() = runTest {
        val pairId = insertPair()
        val entries = listOf(
    ...
    @Test
    fun `getForPair only returns entries for the given pairId`() = runTest {
    ...
    @Test
    fun `getForPair returns empty list when no entries exist`() = runTest {
        val pairId = insertPair()
    ...
    @Test
    fun `delete removes the specified entry`() = runTest {
        val pairId = insertPair()
    ...
    @Test
    fun `clearForPair removes all entries for the given pair`() = runTest {
        val pairId = insertPair()
    ...
    @Test
    fun `clearForPair does not affect entries for other pairs`() = runTest {
        val pair1 = insertPair()
        val pair2 = insertPair()
    ...
    @Test
    fun `cascade delete removes entries when parent pair is deleted`() = runTest {
        val pairId = insertPair()
        localIndexDao.upsertAll(
            listOf(
                buildEntry(pairId, "a.txt"),
