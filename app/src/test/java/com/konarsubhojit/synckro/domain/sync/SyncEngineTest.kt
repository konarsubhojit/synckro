...
class SyncEngineTest {
    ...
    @Test
    fun `FAKE provider with KEEP_BOTH policy writes ConflictRecord for forced conflicts`() = runTest {
        coEvery { mockConflictRepo.getResolvedForPair(any()) } returns emptyList()
    ...
    @Test
    fun `FAKE provider applies pending resolutions on next run`() = runTest {
        coEvery { mockConflictRepo.getResolvedForPair(any()) } returns listOf(
            ConflictRecord(
                id = 99L,
                pairId = 1L,
