...
class RetryPolicyTest {
    ...
    fun `succeeds on first attempt without retrying`() = runTest {
        ...
        val result = withRetry(maxAttempts = 3, initialDelayMs = 0) {
    ...
    @Test
    fun `AuthenticationRequired is not retried`() = runTest {
        val calls = AtomicInteger(0)
        try {
    ...
    @Test
    fun `AuthenticationFailed is not retried`() = runTest {
        val calls = AtomicInteger(0)
        try {
            withRetry(maxAttempts = 5, initialDelayMs = 0) {
                calls.incrementAndGet()
    ...
    @Test
    fun `NotConfigured is not retried`() = runTest {
        val calls = AtomicInteger(0)
        try {
            withRetry(maxAttempts = 5, initialDelayMs = 0) {
                calls.incrementAndGet()
    ...
    @Test
    fun `RateLimited is retried and eventually succeeds`() = runTest {
        val calls = AtomicInteger(0)
        val result = withRetry(maxAttempts = 5, initialDelayMs = 0) {
    ...
    @Test
    fun `onRetry callback receives attempt and exception`() = runTest {
    ...
    @Test
    fun `maxAttempts 1 does not retry on failure`() = runTest {
        val calls = AtomicInteger(0)
        try {
