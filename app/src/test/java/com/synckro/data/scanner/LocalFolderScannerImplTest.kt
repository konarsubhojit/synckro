...
import com.synckro.domain.scan.ScanProgress
...
class LocalFolderScannerImplTest {
    ...
    @Test
    fun `fresh scan of three root-level files adds all to index`() =
        runTest {
    ...
    @Test
    fun `scan stores mimeType on indexed entries`() =
        runTest {
            val pairId = insertPair()
    ...
    @Test
    fun `unchanged file is not upserted and existing hash is preserved`() =
        runTest {
            val pairId = insertPair()
    ...
    @Test
    fun `modified file is re-indexed with null hash`() =
        runTest {
            val pairId = insertPair()
    ...
    @Test
    fun `removed file is deleted from index on second scan`() =
        runTest {
            val pairId = insertPair()
    ...
    @Test
    fun `nested folder fixture of 50 files is indexed correctly`() =
        runTest {
            val pairId = insertPair()
    ...
    @Test
    fun `scanning emits Scanning progress events every PROGRESS_INTERVAL files`() =
        runTest {
            val pairId = insertPair()
    ...
    @Test
    fun `empty tree results in zero-count Done event`() =
        runTest {
            val pairId = insertPair()
    ...
    @Test
    fun `incremental re-scan leaves unchanged entries untouched`() =
        runTest {
            val pairId = insertPair()
    ...
    @Test
    fun `remote columns are preserved during incremental scan`() =
        runTest {
            val pairId = insertPair()
