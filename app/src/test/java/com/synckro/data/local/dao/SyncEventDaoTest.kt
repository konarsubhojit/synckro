...
class SyncEventDaoTest {
    ...
    private fun buildSyncPair(id: Long = 1L) = SyncPairEntity(
        id = id,
    ...
    @Test
    fun `insert and observeAll round-trip for global event`() = runTest {
    ...
    @Test
    fun `observeForPair returns only matching pairId rows`() = runTest {
    ...
    @Test
    fun `observeAll returns newest-first ordering`() = runTest {
        val base = 1_000_000L
    ...
    @Test
    fun `per-pair cap prunes oldest entries beyond MAX_EVENTS_PER_PAIR`() = runTest {
        syncPairDao.insert(buildSyncPair(1L))
    ...
    @Test
    fun `global cap prunes oldest entries beyond MAX_EVENTS_GLOBAL`() = runTest {
