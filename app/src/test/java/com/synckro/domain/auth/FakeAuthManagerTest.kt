...
class FakeAuthManagerTest {
    ...
    @Test
    fun `signIn creates account when configured`() =
        runTest {
    ...
    @Test
    fun `nextSignInResult override is returned only once`() =
        runTest {
    ...
    @Test
    fun `acquireAccessToken needs interactive sign in for unknown account`() =
        runTest {
            val mgr = FakeAuthManager(CloudProviderType.ONEDRIVE)
    ...
    @Test
    fun `signOut removes the account`() =
        runTest {
            val mgr = FakeAuthManager(CloudProviderType.GOOGLE_DRIVE)
    ...
    @Test
    fun `signOut rejects accounts from a different provider`() =
        runTest {
            val mgr = FakeAuthManager(CloudProviderType.ONEDRIVE)
    ...
    @Test
    fun `acquireAccessToken rejects accounts from a different provider`() =
        runTest {
            val mgr = FakeAuthManager(CloudProviderType.ONEDRIVE)
            val stray = Account("x", CloudProviderType.GOOGLE_DRIVE, "x", null)
