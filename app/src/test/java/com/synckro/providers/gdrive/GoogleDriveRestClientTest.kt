...
class GoogleDriveRestClientTest {
    ...
    @Test
    fun `changesSince null fetches startPageToken and returns empty changes`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .setResponseCode(200)
    ...
    @Test
    fun `changesSince with token returns modified and removed items`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .setResponseCode(200)
    ...
    @Test
    fun `changesSince follows nextPageToken before returning newStartPageToken`() =
        runTest {
    ...
    @Test
    fun `changesSince treats trashed file as a removedId in provider mapping`() =
        runTest {
    ...
    @Test
    fun `uploadNew creates session and uploads all chunks`() =
        runTest {
    ...
    @Test
    fun `updateContent initiates session with PATCH and uploads content`() =
        runTest {
    ...
    @Test
    fun `uploadChunked retries chunk on 429 and honours Retry-After`() =
        runTest {
            val uploadUrl = server.url("/upload-session").toString()
            val totalSize = 512L
            val fileBytes = ByteArray(totalSize.toInt()) { it.toByte() }
     ...
     */
    @Test
    fun `uploadChunked resumes from server offset after network drop`() =
        runTest {
            val chunkSize = GoogleDriveRestClient.CHUNK_SIZE
            val totalSize = (chunkSize * 3 / 2).toLong()
            val fileBytes = ByteArray(totalSize.toInt()) { it.toByte() }
    ...
    @Test
    fun `download returns response body stream`() =
        runTest {
    ...
    fun `download throws DriveApiException on 404`() =
        runTest {
    ...
    @Test
    fun `list returns items for root when folderId is null`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .setResponseCode(200)
    ...
    @Test
    fun `list follows nextPageToken for paginated results`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .setResponseCode(200)
    ...
    @Test
    fun `createFolder sends correct request and returns folder`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .setResponseCode(200)
    ...
    @Test
    fun `delete with useTrash=true sends PATCH with trashed=true`() =
        runTest {
    ...
    @Test
    fun `delete with useTrash=false sends DELETE request`() =
        runTest {
    ...
    @Test(expected = DriveApiException::class)
    fun `delete throws DriveApiException on 404`() =
        runTest {
            client.useTrash = false
            server.enqueue(MockResponse().setResponseCode(404).setBody("""{"error": {"code": 404}}"""))
    ...
    @Test
    fun `getMetadata retries on 503 and succeeds`() =
        runTest {
