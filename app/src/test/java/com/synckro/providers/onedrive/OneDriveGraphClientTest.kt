...
class OneDriveGraphClientTest {
    ...
    @Test
    fun `changesSince null calls delta with deltaToken=latest and returns empty changes`() =
        runTest {
    ...
    @Test
    fun `changesSince with deltaLink returns changed and deleted items`() =
        runTest {
    ...
    @Test
    fun `changesSince follows nextLink pages before returning deltaLink`() =
        runTest {
    ...
    @Test
    fun `uploadNew large file creates session and uploads all chunks`() =
        runTest {
    ...
    fun `download throws GraphApiException on 404`() =
        runTest {
    ...
    @Test
    fun `list follows nextLink for paginated results`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .setResponseCode(200)
    ...
    @Test
    fun `createFolder sends correct request and returns folder item`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .setResponseCode(201)
    ...
    @Test
    fun `delete sends DELETE request and succeeds on 204`() =
        runTest {
    ...
    @Test(expected = GraphApiException::class)
    fun `delete throws GraphApiException on 404`() =
        runTest {
            server.enqueue(MockResponse().setResponseCode(404).setBody("""{"error": "itemNotFound"}"""))
