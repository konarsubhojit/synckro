...
class FakeRemoteEnumeratorTest {
    ...
    @Test
    fun `enumerate null returns empty baseline with current log size as token`() = runTest {
    ...
    @Test
    fun `enumerate after baseline returns subsequent upserts as MODIFY`() = runTest {
        val provider = FakeCloudProvider()
        val enumerator = FakeRemoteEnumerator(provider)
    ...
    @Test
    fun `enumerate emits DELETE when a file is removed`() = runTest {
        val provider = FakeCloudProvider()
        val enumerator = FakeRemoteEnumerator(provider)
    ...
    @Test
    fun `enumerate is incremental — successive calls only return new entries`() = runTest {
        val provider = FakeCloudProvider()
        val enumerator = FakeRemoteEnumerator(provider)
