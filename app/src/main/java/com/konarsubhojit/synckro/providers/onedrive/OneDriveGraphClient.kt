...
class OneDriveGraphClient @Inject constructor(
     ...
     */
    internal suspend fun list(token: String, folderId: String?): List<GraphDriveItem> {
    ...
    internal suspend fun getMetadata(token: String, id: String): GraphDriveItem {
    ...
    internal suspend fun uploadNew(
        ...
        return if (size in 0..SMALL_FILE_THRESHOLD) {
     ...
     */
    internal suspend fun createFolder(token: String, parentId: String, name: String): GraphDriveItem {
    ...
    internal suspend fun changesSince(
        ...
        val startUrl = if (deltaToken == null) {
     ...
     */
    private suspend fun createUploadSession(token: String, sessionEndpoint: String): String {
    ...
    private suspend fun queryUploadStatus(uploadUrl: String): Long? {
        return try {
            ...
            status.nextExpectedRanges.firstOrNull()?.substringBefore('-')?.toLongOrNull()
        } catch (e: Exception) {
