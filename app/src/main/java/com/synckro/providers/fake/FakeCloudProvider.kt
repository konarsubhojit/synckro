...
class FakeCloudProvider : CloudProvider {
     ...
     *
     * The created folder has empty content, no `size`, `eTag`, `contentHash`, or `mimeType`, and an autogenerated
     * `id` and `lastModifiedMs`. A corresponding creation entry is appended to the provider's change log.
     *
     ...
     * is always `false`.
     *
     * Per the [CloudProvider.changesSince] token contract, this decimal-index format is an
     * internal implementation detail of this fake and MUST NOT be relied upon by callers —
     * treat `token`/`nextToken` as opaque, exactly as real providers' deltaLink/pageToken values are.
     *
