...
Microsoft (MSAL) authentication work in CI-built debug and testing-release
APKs.
...
> **Security note**: In this repository, the same keystore is intentionally
> reused for debug and testing-release CI artifacts. This is for internal/dev
> testing only. Do **not** use this key for Play Store production signing.
...
5. *(Recommended for testing release builds too)* Create a second Android OAuth
   client with package name `com.synckro` and the same SHA-1 fingerprint.

6. Make sure the **Google Drive API** (and, optionally, the
...
5. *(Recommended for testing release builds too)* Add another Android platform
   entry with package name `com.synckro` and the same signature hash.
   Azure will show:

   ```text
   msauth://com.synckro/<base64-hash>
   ```
...
>   host, or empty path) → **`assembleDebug` / `assembleRelease` fail at
>   configuration time**.
> - `MSAL_REDIRECT_URI` host must match the build variant application ID:
>   `com.synckro.debug` for debug, `com.synckro` for release.
> - CI reads one `MSAL_REDIRECT_URI` secret and rewrites only the host to
>   `com.synckro` when assembling the testing release artifact, while keeping
>   the same path/hash and secret names.
