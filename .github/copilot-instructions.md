...
- This repository is an Android app (`:app`) written in Kotlin with Jetpack
  Compose, Hilt, Room, and WorkManager, plus the platform-free `:core:domain`
  Kotlin JVM module.
...
- Keep sync-domain logic platform-free when possible. `SyncDiffer` in
  `core/domain/src/main/java/com/synckro/domain/sync/` is intentionally
  pure Kotlin and should stay unit-testable without Android dependencies;
  `:core:domain` must never gain `android.*` dependencies.
...
- When changing sync behaviour, add or update unit tests under `app/src/test/`
  (or `core/domain/src/test/` for classes that live in `:core:domain`).
